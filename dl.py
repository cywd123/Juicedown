...
parser.add_argument('-l', '--level', default='INFO') # DEBUG, INFO, WARNING, ERROR, CRITICAL
parser.add_argument('-r', '--retry-num', type=int, default=3)
parser.add_argument('-id', '--twfid', required=True)
...
class ChunkDownload:
    def __init__(self, depot_downloader, mapping, cookies):
        self.depot_downloader = depot_downloader
        self.depot_downloader.web.cookies.update(cookies)
    ...
    def get_chunk(self, chunk_id):
        ...
        while True:
            ...
            #self.log.info(f'{self.path} {chunk_id} {url}')
            try:
...
class DepotDownloader:
    def __init__(self, manifest_path, depot_key, cookies, thread_num=32, save_path=None, servers=None,
    ...
    def download(self):
        ...
        with Pool(int(self.thread_num)) as pool:
            ...
            for mapping in self.manifest.payload.mappings:
                ...
                d = ChunkDownload(self, mapping, self.cookies)
    ...
    if manifest_path_depot_key_dict:
        ...
        for manifest_path, depot_key in manifest_path_depot_key_dict.items():
            if manifest_path and depot_key:
                d = DepotDownloader(manifest_path, depot_key,cookies, args.thread_num, save_path, server_set, level,
